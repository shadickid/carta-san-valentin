...
const ValentineCard = () => {
  // Estados principales
  const [viewMode, setViewMode] = useState('calendar'); // 'calendar' o 'reading'
  ...
  const startDate = new Date(2026, 1, 16); // 16 de febrero 2026
  ...
  const isCardUnlocked = (cardId) => {
    ...
    const cardNumber = cardId - 15;
    return cardNumber <= daysAfterStart + 1;
  };

  const getUnlockDate = (cardId) => {
    if (cardId <= 15) return null;
    const daysToAdd = cardId - 16;
  ...
  const markAsRead = (cardId) => {
    if (!readCards.includes(cardId)) {
      const updated = [...readCards, cardId];
      setReadCards(updated);
  ...
  const isCardNew = (cardId) => {
    ...
    if (readCards.includes(cardId)) return false;
    const unlockDate = getUnlockDate(cardId);
    if (!unlockDate) return false;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    unlockDate.setHours(0, 0, 0, 0);
    const daysSinceUnlock = Math.floor((today - unlockDate) / (1000 * 60 * 60 * 24));
    return daysSinceUnlock <= 2;
  };
  ...
  const getUniqueCharacters = (usedIds) => {
    ...
    return selected.map((char, idx) => ({ ...char, delay: 0.1 + idx * 0.1 }));
  };
  ...
  // GENERACIÓN DE LAS 45 CARTAS
  const generateCards = () => {
    const cards = [];
    let used = [];
    const monthNames = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];

    // CARTAS ORIGINALES 1-15
    // Carta 1
    ...
    cards.push({
      ...
      theme: kuromiMode ? kuromiThemes.kawaii : themes.blackPink, characters: chars1, musicTimestamp: 0
    });
    ...
    cards.push({
      ...
      theme: kuromiMode ? kuromiThemes.pastel : themes.horror, characters: chars2, musicTimestamp: 0
    });
    ...
    cards.push({
      id: 3, title: "Nuestros Momentos 💕",
      message: "Cada foto guarda un recuerdo especial de nosotros.",
      theme: kuromiMode ? kuromiThemes.kawaii : themes.blackPink, characters: chars3,
      musicTimestamp: 0, isPhotoGallery: true, photos: photoGalleries.gallery1
    });
    ...
    cards.push({
      ...
      theme: kuromiMode ? kuromiThemes.lavender : themes.purple, characters: chars4, musicTimestamp: 0
    });
    ...
    cards.push({
      id: 5, title: "Más Recuerdos 🌸",
      message: "Momentos que siempre llevaré en mi corazón.",
      theme: kuromiMode ? kuromiThemes.magenta : themes.gothic, characters: chars5,
      musicTimestamp: 0, isPhotoGallery: true, photos: photoGalleries.gallery2
    });
    ...
    cards.push({
      ...
      theme: kuromiMode ? kuromiThemes.magenta : themes.gothic, characters: chars6, musicTimestamp: 124
    });
    ...
    cards.push({
      id: 7, title: "Mi pequeño microbio",
      message: adultMode 
    ...
    originalCards.forEach(card => {
      ...
      cards.push({
        ...card,
        theme: kuromiMode ? kuromiThemes.kawaii : card.theme,
        characters: chars,
        musicTimestamp: 0
      });
    });
    ...
    cards.push({
      id: 14, title: `${daysTogether} Días Juntos 💕`,
    ...
    // Carta 15 - Foto regalo
    const chars15 = getUniqueCharacters(used);
    cards.push({
      id: 15, title: "Para Mi AMOR 💝",
      ...
      theme: kuromiMode ? kuromiThemes.kawaii : themes.blackPink, characters: chars15,
      musicTimestamp: 0, isPhotoCard: true
    });

    // CARTAS 16-45 - 30 NUEVAS CON TEMAS VARIADOS
    const newCards = [
  ...
  // Marcar como leída
  useEffect(() => {
    if (viewMode === 'reading' && cardsData[selectedIndex]) {
      markAsRead(cardsData[selectedIndex].id);
    }
  }, [selectedIndex, viewMode, cardsData]);
  ...
  const unlockedIndex = unlockedCards.findIndex(c => c.id === cardId);

  if (unlockedIndex !== -1) {
    setSelectedIndex(unlockedIndex);
    setViewMode('reading');
  }
};
useEffect(() => {
        ...
        <div className={`min-h-screen min-h-[100dvh] bg-gradient-to-br ${viewMode === 'calendar' ? 'from-black via-pink-950 to-black' : theme.bg} transition-all duration-700`}>
          ...
          <div className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-md border-b border-white/10">
            <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
              ...
              <motion.button
                onClick={() => setViewMode(viewMode === 'calendar' ? 'reading' : 'calendar')}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                ...
                {cardsData.map((card) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative aspect-[3/4] rounded-2xl border-2 overflow-hidden cursor-pointer transition-all ${
                        unlocked 
                          ? 'border-pink-500/50 shadow-lg shadow-pink-500/20 hover:shadow-pink-500/40' 
                          : 'border-white/10 opacity-50'
                      }`}
                    >
                      {/* Fondo de la carta */}
                      <div className={`absolute inset-0 bg-gradient-to-br ${
                        unlocked ? card.theme.cardBg : 'from-zinc-900/80 to-zinc-950/80'
                      } backdrop-blur-sm`} />

                      {/* Número de carta */}
                      <div className="absolute top-2 left-2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center">
                        <span className="text-white text-sm font-bold">{card.id}</span>
                      </div>
                      ...
                      <div className="relative h-full flex flex-col items-center justify-center p-4">
                        {unlocked ? (
                          <>
                            ...
                            <h3 className="text-white text-center text-sm font-bold line-clamp-2">
                              {card.title}
                            </h3>
                          </>
                        ) : (
                          <>
                            ...
                            {unlockDate && (
                              <p className="text-white/60 text-xs text-center">
                                Se desbloquea<br/>{formatDate(unlockDate)}
                              </p>
                            )}
                          </>
                        )}
                      ...
                      {/* Efecto de brillo para cartas desbloqueadas */}
                      {unlocked && (
                        <motion.div
          ...
          {/* VISTA DE LECTURA */}
          {viewMode === 'reading' && (
            <div className={`min-h-screen flex items-center justify-center p-3 sm:p-4 lg:p-8 overflow-hidden relative transition-all duration-700 pt-20`}>
                            ...
                            {selectedIndex === index && (
                              <div className={`bg-gradient-to-br ${card.theme.cardBg} backdrop-blur-xl rounded-2xl sm:rounded-3xl lg:rounded-[2.5rem] border ${card.theme.border} ${card.theme.shadow} shadow-2xl overflow-hidden transition-all duration-300`}>
                                ...
                                <div className={`bg-gradient-to-r ${card.theme.characterBg} p-4 sm:p-6 lg:p-10 text-center border-b ${card.theme.border}`}>
                                  ...
                                  <div className={`mt-3 sm:mt-4 h-px w-32 sm:w-40 mx-auto ${
                                        ...
                                        {card.photos.map((photo, idx) => (
                                          ...
                                          >
                                            ...
                                            <div className="relative w-full h-full rounded-xl overflow-hidden border-2 border-pink-300/50 shadow-lg">
                                              <img src={photo} alt={`Momento ${idx + 1}`} className="w-full h-full object-cover" />
                                            </div>
                                          </motion.div>
                                        ))}
                                      </div>
